...
class GoogleSpeechWrapper:
    ...
    async def start_listen(client_id: str):
        ...
        config = speech.RecognitionConfig(encoding=GoogleSpeechWrapper.encoding_map[client.audio_config['encoding']],
    ...
    async def start_recognition_stream(sio, client_id: str, config: Dict):
        if client_id not in clients:
            clients[client_id] = ClientData(threading.Thread(target=lambda: asyncio.run(GoogleSpeechWrapper.start_listen(client_id))), sio, config)
